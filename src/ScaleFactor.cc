...
ScaleFactor::~ScaleFactor()
{
  ...
  SafeDelete(histCentral3D_);
  SafeDelete(histShiftUp3D_);
  SafeDelete(histShiftDown3D_);
}
...
void ScaleFactor::RegisterGlobalFunction(int dimension)
{
  ...
  if ( dimension == 1 ) {
    ...
    TString fDecl(Form("double %s(double x, double y, double z) { auto ptr = (ScaleFactor*) gROOT->GetListOfSpecials()->FindObject(\"%s\"); return ptr->Evaluate3D(x, y, z); }", GetName(), GetName()));
    interpreter->Declare(fDecl);
  }
  else {
...
void ScaleFactor::SetOverflowBins(TH3D * hist, OverflowBehavior b)
{
  if ( b == AsInHist ) return;
  else if ( b == NearestEntry ) {
    // TODO: a fair bit of work
