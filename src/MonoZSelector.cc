...
void MonoZSelector::SetupNewDirectory()
{
  ...
  preSelection->Branch("ZeppenfeldLeadLep", &eventList.ZeppenfeldLeadLep, 102400000);
  preSelection->Branch("ZeppenfeldTrailLep", &eventList.ZeppenfeldTrailLep, 102400000);
  ...
  preSelection->Branch("xMet", &eventList.xMet, 102400000);  
  preSelection->Branch("genBJets", &eventList.genBJets, 102400000);
  preSelection->Branch("HT", &eventList.HT, 102400000);
  preSelection->Branch("HTF", &eventList.HTF, 102400000);
  preSelection->Branch("JetPtRatio", &eventList.JetPtRatio, 102400000);
  preSelection->Branch("Rpt", &eventList.Rpt, 102400000);
  preSelection->Branch("JetEtaMultiplicity", &eventList.JetEtaMultiplicity, 102400000);
  preSelection->Branch("DPTOZ", &eventList.DPTOZ, 102400000);
  preSelection->Branch("CJVPt", &eventList.CJVPt, 102400000);
  preSelection->Branch("CJVPtSum", &eventList.CJVPtSum, 102400000);
  preSelection->Branch("genjets", &eventList.genjets, 102400000);
  preSelection->Branch("DeltaPhiLeadingJetMet", &eventList.DeltaPhiLeadingJetMet, 102400000);
...
void MonoZSelector::ProcessSystematic(EventTupleUtil::Systematic systematic)
{
  ...
  const float balance = metForCuts.R()/Zcand.Pt();  
  ...
  //Now subleading
     ...
     if (goodJets.size() > 2){
        for(size_t i=0; i<goodJets.size(); ++i) {
            if (goodJets[i].pt() > Third && goodJets[i].pt() < subleading) {
              Third = goodJets[i].pt();
              ThirdJet = goodJets[i];
            }
        }
  ...
  //CJV variables and HT
  float H_T = 0;
  float CJV_Pt_Sum = 0;
  float CJV_Pt = 0;
  for(size_t i=0; i<goodJets.size(); ++i) {
      H_T = H_T + goodJets[i].pt();
      if(leadingJet.Eta()>0 && goodJets[i].Eta() < leadingJet.Eta()){
         if(goodJets[i].Eta() > subleadingJet.Eta()){
             if( goodJets[i].pt()>CJV_Pt) CJV_Pt = goodJets[i].pt();
             CJV_Pt_Sum = CJV_Pt_Sum + goodJets[i].pt();
         }else{
             //Dont fill
         } 
      }else if(leadingJet.Eta()<0 && goodJets[i].Eta() > leadingJet.Eta()){
         if(goodJets[i].Eta() < subleadingJet.Eta()){
             if( goodJets[i].pt()>CJV_Pt) CJV_Pt = goodJets[i].pt();
             CJV_Pt_Sum = CJV_Pt_Sum + goodJets[i].pt();
         }else{
  ...
  const float zeppenfeld = Zcand.Eta()-(leadingJet.Eta()+subleadingJet.Eta())/2;
  const float zeppenfeldLeadingLepton = leadingLep.p4.Eta()-(leadingJet.Eta()+subleadingJet.Eta())/2;
  const float zeppenfeldTrailingLepton = trailingLep.p4.Eta()-(leadingJet.Eta()+subleadingJet.Eta())/2;
  //end jet work
  ...
  const float x_Z = Zcand.Et() / x_denom30;
  const float x_Met = metForCuts.R() / x_denom30;
  ...
  float R_pt = -1.0;
  //a last set of ratio variables using what we have already created
  const float Jet_pt_Ratio = subleadingJet.pt() / leadingJet.pt();
  if (goodJets.size() >= 2) R_pt = leadingLep.p4.Pt() * trailingLep.p4.Pt() /(leadingJet.pt() * subleadingJet.pt());
  const float Jet_etas_multiplied = leadingJet.Eta() * subleadingJet.Eta();
  const float HT_F = (leadingJet.pt() + subleadingJet.pt()) / H_T;
  //const float dPT_OZ = (leadingJet.pt() - subleadingJet.pt()) / Zcand.Pt();
  const float dPT_OZ = (leadingJet + subleadingJet).pt() / Zcand.Pt();
  
  //Now some DeltaPhi variables for Met.
  float deltaphiclosestjetmet = 10.0;
  float deltaphifarthestjetmet = -1.0;
  float etathirdjet = -10.0;
  const float deltaphileadingjetmet = fabs(DeltaPhi(leadingJet, metForCuts));
  for(size_t i=0; i<goodJets.size(); ++i) {
      if (deltaphiclosestjetmet > fabs(DeltaPhi(goodJets[i], metForCuts))) deltaphiclosestjetmet = fabs(DeltaPhi(goodJets[i], metForCuts));
      if (deltaphifarthestjetmet < fabs(DeltaPhi(goodJets[i], metForCuts))) deltaphifarthestjetmet = fabs(DeltaPhi(goodJets[i], metForCuts));
  }
  if (goodJets.size() > 2){
      etathirdjet = ThirdJet.eta();
  } 

  if ( isNominal  && !skipSystematics && goodLeptons.size() >=2){// && isRealData() && !skipSystematics ) {
    ...
    eventList.ZeppenfeldLeadLep = zeppenfeldLeadingLepton;
    eventList.ZeppenfeldTrailLep = zeppenfeldTrailingLepton;
    eventList.nBjets = nBjetsCSVmedium;
    eventList.xJet = x_jet30;
    eventList.xZ = x_Z;
    eventList.xMet = x_Met;
    eventList.genBJets = genBjets;
    eventList.HT = H_T;
    eventList.HTF = HT_F;
    eventList.JetPtRatio = Jet_pt_Ratio;
    eventList.Rpt = R_pt;
    eventList.JetEtaMultiplicity = Jet_etas_multiplied;
    eventList.DPTOZ = dPT_OZ;    
    eventList.CJVPt = CJV_Pt;
    eventList.CJVPtSum = CJV_Pt_Sum;
    eventList.genjets = genJets;
    eventList.DeltaPhiLeadingJetMet = deltaphileadingjetmet;
