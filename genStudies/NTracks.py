...
def Histo (events):
   ...
   histogram = ROOT.TH1F("NTracks per Jet", "NTracks per Jet", 60, 0, 61)
...
#a               if i2 > 4: 
           ...
           histogram.Fill(count)       #histogram.Fill(IPSig)
   return histogram;
...
histogram.GetXaxis().SetTitle("NTracks per Jet")
histogram.Draw()
...
#legend.AddEntry(histogram,"ctau=1 m_{\chi2}=1","f")
legend.AddEntry(histogram2,"top","f")
legend.AddEntry(histogram3,"ctau=1 m_{\chi2}=150","f")
